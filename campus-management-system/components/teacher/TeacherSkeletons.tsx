...
export function TeacherPageHeaderSkeleton({
  hero = false,
  showAction = false,
  showMeta = false,
  className,
...
export function TeacherFilterBarSkeleton({
  filters = 3,
  showSummary = false,
  className,
...
export function TeacherEventSnapshotSkeleton({
  rows = 3,
  className,
    ...
    <div className={clsx("space-y-3", className)}>
      {Array.from({ length: rows }).map((_, index) => (
        <Card
          key={index}
          shadow="none"
          className="border border-border/70 bg-white/90 shadow-[var(--shadow-soft)]"
        >
...
export function TeacherDetailPanelSkeleton({
  rows = 6,
  className,
}: ClassNameProps & { rows?: number }) {
  return (
